...
namespace Min_Helpers.Win32ApiHelper
{
    /// <summary>
    /// Monitor
    /// </summary>
    public class Monitor
    {
        ...
        private static extern int GetSystemMetrics(ESystemMetric smIndex);
        ...
        private static extern IntPtr MonitorFromPoint(Point pt, EMonitorOptions dwFlags);

        /// <summary>
        /// EMonitorOptions
        /// </summary>
        public enum EMonitorOptions : int
        {
            /// <summary>
        ...
        /// Get Metrics
        /// </summary>
        public List<KeyValuePair<int, int>> GetMetrics
        {
            get
            {
                try
                {
                    List<KeyValuePair<int, int>> metrics = new List<KeyValuePair<int, int>>();

                    foreach (ESystemMetric systemMetric in Enum.GetValues(typeof(ESystemMetric)))
                    {
                        int data = GetSystemMetrics(systemMetric);

                        metrics.Add(new KeyValuePair<int, int>(Convert.ToInt32(systemMetric), data));
                    }
        ...
        public int GetCount
        {
            get
            {
                try
                {
                    int count = GetSystemMetrics(ESystemMetric.SM_CMONITORS);
        ...
        public Point GetLimit
        {
            get
            {
                try
                {
                    int limitX = GetSystemMetrics(ESystemMetric.SM_CXVIRTUALSCREEN),
                        limitY = GetSystemMetrics(ESystemMetric.SM_CYVIRTUALSCREEN);

                    Point limit = new Point(limitX, limitY);
        ...
        public int GetPrimary
        {
            get
            {
                try
                {
                    List<bool> primarys = new List<bool>();

                    MonitorEnumDelegate callback = (IntPtr hMonitor, IntPtr hdcMonitor, ref Rect lprcMonitor, IntPtr dwData) =>
                    {
                        primarys.Add((lprcMonitor.left == 0) && (lprcMonitor.top == 0));
                    ...
                    int index = (primarys.FindIndex(n => n == true) + 1);
        ...
        public PointF GetDpi
        {
            get
            {
                try
                {
                    using (Graphics graphics = Graphics.FromHwnd((IntPtr)EDpiType.Effective))
                    {
